...
def make_textures():
    ...
    ashlar("stone.jpg", 0.5, (0.6, 1.3), [(138, 118, 98), (128, 110, 92), (148, 128, 106), (120, 104, 88), (160, 140, 118),
                                         (132, 114, 96), (112, 98, 84)], (176, 162, 144), 7.0, 3)
    ashlar("cream.jpg", 0.45, (0.9, 1.6), [(236, 222, 196), (230, 214, 186), (240, 228, 204), (226, 212, 188)], (214, 198, 170), 4.0, 4)
    ...
    for r in range(int(N / th) + 2):
        ...
        for c in range(int(N / tw) + 2):
            x0 = c * tw - off; y0 = r * th
            base = [(72, 98, 150), (64, 90, 140), (80, 108, 160), (58, 82, 130), (88, 116, 164)][g.randrange(5)]
...
def materials():
    ...
    if not (TEX_DIR / "stone.jpg").exists():
        make_textures()
    M["stone"] = image_material("cc_stone", "stone.jpg", 0.9, 0.5, (0.36, 0.31, 0.26))
    M["cream"] = image_material("cc_cream", "cream.jpg", 0.55, 0.15, (0.85, 0.79, 0.68))
    M["lilac"] = M["cream"]                                 # (the shared helpers call the wall material "lilac")
    M["roof"] = image_material("cc_slate", "slate.jpg", 0.45, 0.4, (0.22, 0.31, 0.50))
    mat, nt, b = _principled("cc_trim", (0.93, 0.88, 0.78), 0.5); _mottle(nt, b, (0.93, 0.88, 0.78), 6, 0.93, 0.03); M["trim"] = mat
    M["gold"] = P("cc_gold", (0.90, 0.70, 0.30), 0.22, Metallic=1.0)
...
def build_base(P):
    ...
    lathe(P, "trim", [(0.9, 0), (1.2, 0), (1.2, 0.2), (0.9, 0.2)], 24, Mr @ T(-6.5, -0.1, 7.2) @ R(math.pi / 2, "X"))
    lathe(P, "glass", [(0, 0), (0.9, 0), (0.9, 0.05), (0, 0.05)], 24, Mr @ T(-6.5, -0.05, 7.2) @ R(math.pi / 2, "X"))
    for u in (-9.0, 6.0, 9.5):                                                  # the smaller arches of the rear arcade
        poly_prism(P, "trim", Mr, [(x, z) for x, z in arch_ring(u, 2.4, 3.0, 0.35)], -0.3, 0.05)
        poly_prism(P, "glass", Mr, [(u - 1.2, 0.0), (u + 1.2, 0.0)] + pointed(u - 1.2, u + 1.2, 3.0)[1:-1], 0.04, 0.05)
...
def build_palace(P):
    ...
    for (M, L) in BC.side_frames(-3.9, 8.4, 4.0, 26.0):
        n = max(1, int(L / 2.6))
        for k in range(n):
            gothic_window(P, M, (k + 0.5) * L / n, zb + 2.0, 1.1, 4.2)
            BC.window(P, M, (k + 0.5) * L / n, zb + 8.8, 0.7, 1.8, "pointed", 0.1)
        balustrade(P, M @ Matrix.Translation((0, 0.3, 0)), 0.0, L, zb + 0.2, 0.8)
    ...
    for k in range(7):
        gothic_window(P, Mr, 1.6 + k * 3.1, zb + 1.6, 1.3, 4.6)
    ...
    for u_ in (-6.0, -1.0, 7.0):
        gothic_window(P, face(0, 33.1, math.pi) @ Matrix.Translation((0, 0, 0)), -u_, 21.6, 0.8, 2.2)
...
def build_forecourt(P):
    """The forecourt stage in front of the gate (Sketchfab model, photos): a raised paved terrace with a balustrade,
    steps down to the plaza, banner poles; the moat round the castle base."""
    I = Matrix.Identity(4)
    cube(P, "stone", I, -16.0, 20.0, -13.0, 1.0, -0.8, 1.0)
    cube(P, "paving", I, -15.8, 19.8, -12.8, 1.0, 1.0, 1.05)
    lathe(P, "red", [(0, 1.05), (3.2, 1.05), (3.2, 1.07), (0, 1.07)], 40, T(0.0, -6.5, 0))          # the round motif on the stage
    lathe(P, "paving", [(0, 1.07), (2.4, 1.07), (2.4, 1.08), (0, 1.08)], 40, T(0.0, -6.5, 0))
    for (x0, x1, y0, y1) in ((-16.0, -4.0, -13.2, -12.8), (4.0, 20.0, -13.2, -12.8), (-16.2, -15.8, -13.0, 0.5), (19.8, 20.2, -13.0, 0.5)):
        if y1 - y0 < 1:
            balustrade(P, face(x0, -13.0, 0), 0.0, x1 - x0, 1.05, 0.9)
        else:
            balustrade(P, face(x0 + 0.2, 0.5, -math.pi / 2), 0.0, 13.5, 1.05, 0.9)
    for k in range(5):                                                          # the steps down in the middle
        cube(P, "trim", I, -4.0, 4.0, -13.0 - (k + 1) * 0.35, -13.0 - k * 0.35, -0.8, 1.0 - (k + 1) * 0.2)
    for x_ in (-14.0, -9.0, 9.0, 14.0, 18.5):                                   # banner poles
        lathe(P, "gold", [(0, 1.05), (0.07, 1.05), (0.05, 8.0), (0.12, 8.2), (0, 8.5)], 8, T(x_, -11.8, 0))
        cube(P, "banner_a" if x_ < 0 else "banner_b", I, x_ + 0.05, x_ + 0.08, -11.8 - 0.55, -11.8 + 0.55, 4.0, 7.6)
    cube(P, "water", I, -30.0, 34.0, -2.0, 48.0, -1.3, -1.25)                  # the moat (under the forecourt and the base)
...
CAMS = {
    "front": ((2.5, -150.0, 2.0), (2.5, 10.0, 24.0), 60),         # from across the plaza, long lens (the photos' view)
    "front_close": ((0.0, -40.0, 1.7), (0.0, 5.0, 15.0), 26),
    "rear": ((-18.0, 58.0, 1.7), (2.0, 20.0, 18.0), 22),          # the Fantasyland side: the gothic arch, the stair
