...
- 2026-09-24 の確認の記録(電車を追うカメラ): 地図のパネルから押すと俯瞰に切り替わって追いかける(3 秒で電車が 29 m 進み、カメラも付いてくる)。4 つの視点、編成の切り替え、追いかけ中のドラッグ、Esc と F、軽量モードでの前面展望(車内から外が見える)、散歩に切り替えたときの解除、前面展望のまま駅に停まるところを確かめた。
...
俯瞰では「電車を追う」(F キー)で、カメラが電車を追いかける(視点は 追いかけ・横・前面展望・上空)。
...
ランドの階段・橋・壁・等高線などは、その機能のレイヤーと「ディズニーランド」の両方がオンのときに出る。リゾートラインの電車は「高架鉄道」と「舞浜駅・周辺」の両方がオンのときに出る。「全体」ボタンは、表示中のレイヤーに合わせて、シーだけ / シー+ランド+舞浜駅の範囲に合わせる。
...
- **表示の切り替え**: 「高架鉄道」と「舞浜駅・周辺」の両方がオンのときだけ出る。
- **電車を追う**(俯瞰): 左上の「電車を追う」、F キー、または「エリア・移動」タブの「〇〇の編成を追う」(地図からでも俯瞰に切り替わる)。上に出るバーで、編成(ブルー / イエロー)と視点を選ぶ。
  - 追いかけ: 斜め後ろの上から。横: 右側から。上空: 150 m 上から。この 3 つは、追いかけている間もドラッグで回す・ホイールで寄ることができ、電車が曲がるとカメラも一緒に回る(上空は回らない)。
  - 前面展望: 先頭車の運転台の上(先端から 2.2 m、目の高さは桁から 2.5 m)から、60 m 先の線路を見る。軽量モードでも、追っている編成の近くの車両だけは詳細版にする(簡略版は窓を塗っていて外が見えないため)。
  - 切り替えは 1.2 秒かけて滑らかに移る。× ・ Esc ・ F でやめる。散歩・地図・ミニマップ・エリア移動・「全体」でもやめる。バーに編成の状態(停車中の駅・速度・次の駅)を出す。
...
| **電車を追う(追いかけ)** | **電車を追う(前面展望・駅に停車中)** |
| ![追いかけ](docs/train/follow_chase.jpg) | ![前面展望](docs/train/follow_cab.jpg) |
