...
- **PC**: マウスでドラッグすると見回せる(カメラだけが回り、キャラクターの向きは変わらない)。W A S D / 矢印キーでカメラの向きを基準に歩き(1.4 m/秒)、キャラクターは進む方向を向く。Shift で走る(6 m/秒。ウッディは全力疾走の動き)。V キーで一人称(目の高さ 1.58 m)と三人称を切り替える。Esc で俯瞰に戻る。
- **スマホ**: 左下のスティックで歩き、画面をドラッグして見回す。右下の「走る」で走る。
- 画面中央の照準の先にある物(建物の名前・エリア・高さ、3D モデルの面、階段、水面、地面の標高)と距離を表示する。上に方位、ミニマップに現在地と向きを出す(足元の高さ・位置の表示は 2026-09-25 に廃止)。ミニマップや「エリア・移動」の一覧で、その場所へ移動できる(建物や水の中なら、近くの地面に降りる)。
