...
パネルの「3Dモデル」欄(3D タブのみ)で、モデルごとに表示を切り替えられる。
...
3D 表示の「人の目線」ボタンで、アクアスフィアから 22 m(東寄り) のプラザに、地球儀を向いて身長 170 cm の人が立ち、歩いて見え方を確かめられる(PC のみ)。

- W A S D / 矢印キーで歩く(1.4 m/秒)、Shift で走る(4 m/秒)、ドラッグで向きを変える。
...
- 画面の左下に、足元の高さ(園路基準)と位置を表示する。

「レイヤー」欄と「3Dモデル」欄は、見出しをクリックして折り畳める(状態は `tds-fold` に保存)。各欄の「すべてオン / すべてオフ」で一括切り替えができる(「重ねて表示」は対象外)。
