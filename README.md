...
| **俯瞰** | 高さつきの枠線と 3D モデルを、ぐるっと回して見る。右下のミニマップを押すと、そこへカメラが飛ぶ。最初に開いたときはオープニング: 操作部品を隠し、東京ディズニーランド・ステーションのアップ(駅のモデルを最初に読み込む)から始まり、周りを回りながら枠線が地面から約 2.8 秒で伸び上がる。伸び切ったらカメラが引いてパーク全体へ(約 4.2 秒。その間にほかの 3D モデルを 1 つずつ読み込んで伸ばして出す)、最後にタイトル「ディズニーアドベンチャー」を出す。画面のどこかをタップすると、いつもの画面に戻る(途中でタップすると、そこで飛ばす)。共有 URL で開いたときは枠線が伸びるだけ。散歩から始めたときと、端末で「視差効果を減らす」がオンのときは演出なし。 |
