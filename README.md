...
| シンデレラ城の 2D 立面図(昼・夕方・夜) | 済(単独のページ。高さと幅は OSM の実測、ほかは推定) | `output/disneysea/castle.html` |
| 優先パーツの作り込み(ミラコスタ、コロンビア号、シンデレラ城の 3D など) | これから | — |
...
- 2026-09-24 の確認の記録(シンデレラ城の 2D 立面図。Chromium のヘッドレス): 昼・夕方・夜と寸法線を、1600×1000 と 390×844(スマホ縦)で表示してコンソールのエラーがないこと。縦画面で尖塔がパネルに隠れないこと。横画面の絵は縦画面向けの変更の前後で同じ(ファイルが一致)。
...
見出しバーのボタン: **共有**(今の視点の URL をコピー。スマホでは共有メニュー)、**軽量**(水の透過・しぶきを切り、解像度を下げる。タッチ端末では最初からオン)、**パネル**。
...
- **エリア・移動**: 地図と俯瞰ではそのエリアへ移動して強調。散歩ではその場所へ移動。アクアスフィア(散歩のスタート地点)、ディズニーランドの各エリア、舞浜駅、ランドのエントランスも選べる(ランドと舞浜駅は地図と俯瞰のみ)。
...
ランドの階段・橋・壁・等高線などは、その機能のレイヤーと「ディズニーランド」の両方がオンのときに出る。リゾートラインの電車は、地図の印を「電車(リゾートライン)」レイヤーで、俯瞰・散歩の電車を 3D モデルの「電車(リゾートライン)」で切り替える。「全体」ボタンは、表示中のレイヤーに合わせて、シーだけ / シー+ランド+舞浜駅の範囲に合わせる。
...
- **表示の切り替え**: 地図の印(とミニマップの点)はレイヤーの「電車(リゾートライン)」、俯瞰・散歩の電車は 3D モデルの「電車(リゾートライン)」で切り替える。ほかの 3D モデルと同じく、レイヤーとは独立(プリセット「枠線のみ」では 3D の電車が消え、「3Dモデルのみ」では地図の印が消える)。
- **電車を追う**(俯瞰): 左上の「電車を追う」、F キー、または「エリア・移動」タブの「1本目(ブルー)を追う」「2本目(イエロー)を追う」(地図からでも俯瞰に切り替わる)。「電車を追う」と F は 1 本目を追う。3D モデルの電車がオフなら、オンに戻す。
...
- ページは `train_model.js` を相対パスで読み込む(公開先では `/train_model.js`)。`tds_outline.html` を単体で開いたときも、同じフォルダにあれば動く。
...
`output/disneysea/castle.html`(公開 URL の `/castle.html`)は、東京ディズニーランドのシンデレラ城を正面(ワールドバザール側)から見た 2D の絵。canvas だけで描いていて、画像ファイルもライブラリも使わない。

- **実測**(OSM の `シンデレラ城`): 高さ 51 m(頂部飾りの先まで)、建物の幅 約 36 m(東西)。絵の縮尺はこの 2 つに合わせた。
- **形**: 岩の土台と堀、灰色の石積みの城壁・胸壁・持ち送り(マチコレーション)、尖頭アーチの通り抜けと落とし格子、橋と街灯、紋章。上は白い城と大広間の尖頭窓、青い円すい屋根(うろこ状の葺き目)と金の帯・頂部飾り、八角形の台の上の中央の尖塔。左右の塔は高さを少し変えてある。
- **推定**: 各塔の位置・太さ・高さ、窓の数と大きさ、城壁の長さ。参考写真は見られなかった(この端末からは Wikimedia Commons に届かない)ので、形は一般に知られている外観から起こした。
- **時間帯**: 昼・夕方(左から夕日、窓の一部が灯る)・夜(投光と窓の明かり、月と星)。夜の窓の明かりは、城を黒で描き直して窓だけ光らせたもう 1 枚を重ねるので、手前の塔に隠れる窓は光らない。
- **表示**: 寸法線(高さ 0〜51 m の目盛りと幅 36 m)、水面の映り込み(帯ごとに横にずらして波を出す)、PNG で保存。
- URL で開き方を指定できる: `?mode=day|dusk|night`、`&dims=1`(寸法線)、`&clean=1`(パネルを隠す。スクリーンショット用)。
- スマホの縦画面では、左右の木を切って城を大きく、パネルの下に描く。

| 昼 | 夕方(寸法線なし) | 夜 |
|---|---|---|
| ![昼](docs/castle/castle_day.jpg) | ![夕方](docs/castle/castle_dusk.jpg) | ![夜](docs/castle/castle_night.jpg) |

![ページ(寸法線あり)](docs/castle/castle_page.jpg)
...
- 配信するのは `.gitignore` の許可リストにあるファイルだけ(`tds_outline.html`、`train.html`、`train_model.js`、`castle.html`、`models/`)。新しいファイルを足したら許可リストにも足す。
...
- 樹林は高さ 7 m の箱なので、水面や建物を隠しやすい。屋根だけの構造物(OSM の `building=roof`。約 59 か所)も地面からの箱になっていて、散歩では壁になる。
- 階段の向き不明が 16 か所(シー 13、ランド 3)。
- 船、夜の照明はまだ作っていない。
- リゾートラインの電車は、推定の走り方をくり返すだけ(実際の時刻表・編成数ではない)。桁と柱は枠線のままで、車両の床(桁から 1 m)は駅のホーム(8 m)より高い。
- ランド・舞浜駅は Blender のシーンに未収録。トイ・ストーリーホテルの建物は OSM にない。
