...
| 電車(リゾートライン)の JS 版と JS / Blender の比較 | JS 版は写真に合わせて車内まで作り込み済み。Blender 版は JS 版と同じ形・寸法に書き直し済み(Blender では未実行) | `output/disneysea/train.html`, `train_blender.py`, `docs/train/spec.md` |
| リゾートラインの電車をモックで走らせる(2 編成が全周を周回、4 駅で停車) | 済(地図は印、俯瞰・散歩は 3D。遠くと軽量モードは簡略版) | `output/disneysea/train_model.js`, `mock_template.html` の `RL` |
...
- [ ] 電車: `blender -b --python train_blender.py -- --cars head --render output/train_head.png` と `-- --cars train --out output/disneysea/models/train.glb` を動かし、`train.html` と見比べる。形・寸法は JS 版(`train_model.js`)と同じにしてある(`tools/blender_smoke.py` が数値を突き合わせる)。車内の作り込みと汚れのベイクは TODO。
...
- **向きと高さ**: 車両ごとに、前後の台車の位置から線路の向きを取る(カーブで車両が折れ曲がる)。桁の上面は、モックの高架(推定 8 m)+ 桁 0.9 m。
...
- `train_model.js` は、`export_mock.py` がページ(`tds_outline.html`)の中に埋め込む(別ファイルを読みに行かない)。`train_model.js` を直したら `python export_mock.py` を実行する。3D の電車を作れなかったときは、画面に理由を出す(地図の印と「電車を追う」のカメラは、それでも動く)。
...
ディズニーリゾートラインの東京ディズニーシー・ステーション(ホームと駅の前後の線路。高架は推定 8 m)、入場ゲートの列(パークエントランス・ノース / サウス、各 12 か所。高さ 1.2 m の枠)、回転式の改札、ゲストリレーション、駅前の通路。
...
- **舞浜駅・周辺**: 舞浜駅(JR 京葉線。高架は推定 10 m)、リゾートラインの全周と 4 駅(高架は推定 8 m)、イクスピアリ、ホテル(ディズニーのホテルと範囲内のオフィシャルホテル)、舞浜駅からランドのエントランスまでの歩道。
...
- 配信するのは `.gitignore` の許可リストにあるファイルだけ(`tds_outline.html`、`train.html`、`train_model.js`、`models/`)。新しいファイルを足したら許可リストにも足す。
...
- リゾートラインの電車は、推定の走り方をくり返すだけ(実際の時刻表・編成数ではない)。桁と柱は枠線のままで、車両の床(桁から 1 m)は駅のホーム(8 m)より高い。
