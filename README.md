...
| **俯瞰** | 高さつきの枠線と 3D モデルを、ぐるっと回して見る。右下のミニマップを押すと、そこへカメラが飛ぶ。最初に開いたときは、枠線が地面から約 1.4 秒で伸び上がり、そのあと 3D モデルを 1 つずつ読み込んで、それぞれ地面から伸ばして出す(読み込み待ちの間も画面が動くように。一度に全部を読むと画面が止まるため)。散歩から始めたときと、端末で「視差効果を減らす」がオンのときは伸ばさない。 |
