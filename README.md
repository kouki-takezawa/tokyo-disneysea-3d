...
| モード | 内容 |
|---|---|
| **地図** | 平面図。ドラッグで移動、ホイール(スマホは 2 本指)で拡大。建物・階段・電車にマウスを乗せる(スマホはタップする)と詳細が出る。ラベルは重なるものを省いて出す(エリア名が優先)。 |
| **俯瞰** | 高さつきの枠線と 3D モデルを、ぐるっと回して見る。右下のミニマップを押すと、そこへカメラが飛ぶ。ページを開くたびにオープニング(俯瞰): 操作部品を隠し、東京ディズニーランド・ステーションのアップ(駅のモデルを最初に読み込む)から始まり、周りを回りながら枠線が地面から約 2.8 秒で伸び上がる。伸び切ったらカメラが引いてパーク全体へ(約 4.2 秒。その間にほかの 3D モデルを 1 つずつ読み込んで伸ばして出す)、最後にタイトル「DISNEY ADVENTURE」を出す(オープニングの間は映画のような上下の黒帯と周辺減光。タイトルは Cinzel の書体で 1 文字ずつ現れて金色に光り、金の弧の先から金の粉が舞う)。画面のどこかをタップすると、いつもの画面に戻る(途中でタップすると、そこで飛ばす)。URL に視点(`#map/…` `#orbit/…` `#walk/…`)があれば、タップのあとその視点へ移る。オープニングの間は URL の視点を書き換えない。3D のライブラリを読み込めないときは地図で開く。 |
...
(夜のモード・軽量モード・高さの強調(高さ ×3)は 2026-09-25 に廃止。端末がダークモードでも、いつも明るい配色で出す。スマホでは描画解像度を自動で 1.5 倍までに抑える。)
