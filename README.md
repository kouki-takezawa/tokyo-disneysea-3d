...
| ディズニーランドと舞浜駅まわり(枠線モックのレイヤー) | 済(建物の高さの大半は推定) | `fetch_disneyland.py`, `ds_disneyland.py` |
...
- レイヤー: 建物・優先パーツ・水面・緑地・樹林・岩場・園路・階段・橋・トンネル・壁・等高線・高架鉄道・ランドマーク・入場口・駅・ディズニーランド・舞浜駅・周辺。
- ディズニーランド(`ds_disneyland.py`): 園の境界・建物(375 棟)・水面・緑地・樹林・園路・ウエスタンリバー鉄道。建物は 7 つのエリア(ワールドバザール / アドベンチャーランド / ウエスタンランド / クリッターカントリー / ファンタジーランド / トゥーンタウン / トゥモローランド)とバックステージ・その他に色分けする。エリアはシーと同じく、OSM の施設名(と名前付きの建物)で決める(`ds_disneyland.LANDS`)。高さは OSM のタグ、シンデレラ城は 51 m、それ以外は推定 12 m(バックステージは 10 m)。
  - 階段・橋・トンネル・壁・等高線もシーと同じ方法で作る(`ds_levels.compute()` をランドの OSM に使う。高さの基準はシーと同じ標高 5.29 m)。階段 24 か所の根拠は DEM で確定 5 / つながりから推定 4 / 長さから推定 12 / 向き不明 3。ランドのこれらは、各レイヤー(階段・橋・壁・等高線など)と「ディズニーランド」の両方がオンのときに出る。要確認の階段の一覧にも入る。
  - OSM の建設工事の区域は点線で示す。旧スペース・マウンテン跡の「トゥモローランド新規アトラクション建設工事」の区域内の建物はトゥモローランド、ウエスタンリバー鉄道の機関庫など裏手の建物は「その他」にする。
- 舞浜駅・周辺: 舞浜駅(JR 京葉線、駅の前後の線路)、ディズニーリゾートラインの全周と 4 駅、イクスピアリ、ホテル(ディズニーのホテルと、範囲内のオフィシャルホテル)、舞浜駅からランドのエントランスまでの歩道。リゾートラインの高架は推定 8 m、京葉線は推定 10 m。
- ランドと舞浜駅のデータは `fetch_disneyland.py` で別ファイルに取る(シーのデータを取り直すと、火山などの元データまで変わってしまうため)。標高はシーと同じ DEM5A のタイルがランドと舞浜駅まで覆っている。
- 「エリア・移動」タブの「ディズニーランド・舞浜駅」から、舞浜駅・ランドのエントランス・各エリアへ移動できる(地図と俯瞰。散歩はシーの中だけ)。
- 「全体」は、表示中のレイヤーに合わせて、シーだけ / シー+ランド+舞浜駅の範囲に合わせる。
- 入場口・駅(`ds_core.entrance_layer()`): ディズニーリゾートラインの東京ディズニーシー・ステーション(ホームと駅の前後の線路。ホームの高さは推定 8 m)、入場ゲートの列(パークエントランス・ノース / サウス、各 12 か所。高さ 1.2 m の枠)、回転式の改札、ゲストリレーション、駅前の通路。散歩ではゲートは当たり判定なしで通り抜けられる。
...
- 確認に使う道具(Blender なしで動く):
  ...
  - `python tools/aerial_overlay.py X0 Y0 X1 Y1 [OUT.jpg]` — モックのデータを国土地理院の航空写真に重ねて、位置を目で確かめる。
