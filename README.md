...
| 東京ディズニーランド全体の地面(駐車場からパークの奥まで。舗装・道路・駐車場・芝・林・岩場・工事中・水面と護岸。建物の場所は作らず、建物を抜ける通路は作る。木は作らない) | 済(Blender なし。純 Python で作り、モックの 3D モデルに) | `ds_tdl_land_ground.py` |
...
python ds_tdl_land_ground.py       # ランド全体の地面 → models/tdl_land_ground.json(同上。約 40 秒)
```
...
- 線路は駅の中も途切れずに通る(桁は `ds_tracks.py` のもの。駅のモデルの桁はモックに書き出さない)。電車が通れるように、駅の両端の壁はなく、1 階は線路の下だけ低くしてある(2026-09-25 に直した)。
...
- **ホテルと駅の間の埋め**: OSM では線だけで面が無いところ(ホテル前のプロムナードと駅の周り)は、OSM の線(道・柵・建物・庭・水・リゾートラインの桁)で囲まれた区画を面として埋める。庭・芝・花壇の区画は縁石つきの植え込み、サービス道路から 6 m 以内(幅 12 m)はアスファルト、残りは赤茶のレンガ(航空写真: 駅前の道は灰色、その両側の歩道はレンガ)。
- **駅の中の通路**: 東京ディズニーランド・ステーションの外形の中にある短い通路 7 本(OSM では tunnel / covered / layer=-1。建物の下と棟の間を抜ける通路)を幅 4 m の石の道にした。駅の外形の下には、Blender の駅の床の高さ(-1.79 m)で石の床を敷いたので、駅の下に空白は出ない。
- **作らないもの**: ホテルとほかの建物の場所(屋根だけの構造物は除く)、水面、地下道・橋(ゲートウェイの歩道橋)・屋根つきの道(駅の中の通路は除く)、階段、公道(浦安市道幹線7号)、遠くの大きな駐車場の通路、エントランス広場(`ds_tdl_ground.py` が作るので、その範囲は切り抜く)。ホテル正面の車寄せの舗装は OSM に面として無いので作っていない。
...
- 約 2 万三角形・1.1 MB(車道 6,700 m²、園路 5,400 m²、広場 3,100 m²、植え込み 22 か所)。

| モックでの表示(俯瞰。枠線のホテルと重ねたもの) |
|---|
| ![ホテル周辺の道](docs/entrance/mock_hotel_ground.jpg) |

### 東京ディズニーランド全体の地面(`ds_tdl_land_ground.py`)と駐車場のレイヤー

ランドの駐車場からパークの奥までの地面。エントランス広場・ホテル周辺と同じ作り方(Blender なし、木は作らない)。

- **範囲**: ランドの外形(OSM way 1282875870。南西の大きな駐車場も含む)と、そこから 30 m 以内の平面駐車場(西のグーフィー・ピノキオ・ティンカーベルなど)。間の細いすき間も 25 m のクロージングで含める。エントランス広場とホテル周辺(作成済み)とシーは除く。
- **種類**(先に当てはまったもの): 水面(アメリカ河・城の堀・池。周りの地面の低い方から 0.6 m 下の平らな面と、そこまで下る護岸) / 舗装(`highway=pedestrian` の面、園路 3.5 m・歩行者道 8 m) / 道路(裏の `service` など 5 m、`tertiary` 以上 7 m) / 駐車場 / 岩場(`scree` `bare_rock` `sand`: ビッグサンダー・マウンテンや島) / 芝・庭 / 林(木は無し) / 工事中 / その他の地面(OSM に種類が無い所。多くは舗装なので、明るい石の色)。
- **建物**: 建物の場所には地面を作らない(屋根だけの構造物と、50 m² 未満の小屋・待合所は地面を続ける。穴が点々と残るため)。**建物を抜ける通路**(`tunnel=building_passage`・`covered=yes`、建物の中に収まる地下通路・アーケード)は、幅 4 m の地面を作る(シンデレラ城の下の通路など)。
- **高さ**: エントランス広場と同じ地形(DEM5A、約 5 m でならす、エントランスの下は平ら)。建物と水面の下は DEM が地面ではないので、周りから補間する。リゾートラインの駅の周りは駅の床(-1.79 m)にそろえる。
- 約 9.3 万三角形・4.7 MB(8 m の格子で切る)。
...
- **推定・未確認**: 幅、色、水位、どの駐車場を含めるか。アトラクションの建物の中の地面は作っていない。お城や山のモデルは、この地面の上に置いてあるだけ。

| モックでの表示(俯瞰) |
|---|
| ![ランド全体の地面](docs/entrance/mock_land_ground.jpg) |
...
- **大きさ(2026-09-25 にユーザーの指示で決め直し)**: 門の正面の扉を高さ 3 m と決め、城全体をその比で縮めた(形の比率は写真のまま)。写真で扉を物差しに確かめ直し、門の扉と両脇のくぼみが約 1.6 倍高すぎたのを直した。結果: 門の屋根の頂 10.6 m、両翼の手すり 13.5 m、本館の扉 約 5 m、天守の胸壁 約 37.5 m、尖塔の先 42.6 m。公表の約 30 m より高いが、扉を物差しにした値を採った。
- **レイヤーとの違い**: 「ディズニーランド」レイヤーの枠線(紫の箱)は、OSM のアトラクションの建物(高さタグなし)を推定 12 m で描いたもの。城のモデルとは高さも形も違う。
...
- **リゾートライン(跨座式モノレール)**: 幅 0.85 m・高さ 1.5 m のコンクリートの桁(カーブでは最大 6° 内側へ傾ける)、約 22 m ごとの継ぎ目と鋼の伸縮板、その下にラッパ形の頭の丸い橋脚と支承、桁の側面の電車線(Blender では碍子の腕木を 1 個 +「配列 + カーブ」で並べる)。リゾートゲートウェイ・ベイサイド・東京ディズニーシーの 3 駅は、あとで駅ごとに作るので、今は入れていない(ユーザーの指示、2026-09-25。ホームの下書きは `build_rl_platforms()` に残し、`RL_STATIONS = False` で外している)。東京ディズニーランド・ステーションの区間は駅のモデルに任せる。
